...
cbor_item_t *network_frame::to_cbor() const {
  ...
  cbor_map_add(root, {.key = cbor_move(cbor_build_string("d")),
  ...
  cbor_map_add(root, {.key = cbor_move(cbor_build_string("i")), .value = cbor_move(ids)});
...
cbor_item_t *network_metadata::to_cbor() const {
  ...
  cbor_map_add(root, {.key = cbor_move(cbor_build_string("codecName")),
                      .value = cbor_move(cbor_build_string(codec_name.c_str()))});

  cbor_map_add(root, {.key = cbor_move(cbor_build_string("codecData")),
                      .value = cbor_move(cbor_build_string(base64_data.c_str()))});
