...
inline cv::Point center(cv::RotatedRect a) { return a.center; }

inline double ordering_value(cv::Rect a) { return a.y; }
inline double ordering_value(cv::RotatedRect a) { return a.center.y; }
...
int closest(std::vector<A> h1, int e2, std::vector<A> h2, double MAX_DISTANCE) {
  ...
  for (int i = 0; i < h1.size(); i++) {
    ...
    if (ordering_value(h1[i]) > ordering_value(a) + MAX_DISTANCE) break;
  }
  if (min > MAX_DISTANCE) return -1;
  return e1;
}
}  // namespace cvbot
}  // namespace video
